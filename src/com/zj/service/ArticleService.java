...
public class ArticleService {
	...
	public List<Map<String, Object>> getAllArticle(){
		...
		try {
			List<Article> articleControl = articleControl.getAllArticle();
			if(articleList != null){
	...
	public List<Map<String, Object>> getPageArticleInfo(Integer articlePresentPage) throws SQLException{
		...
		//文章分页
		Long articleCount = articleControl.queryCountArticle();
		...
		List<Article> pageArticle = articleControl.queryPageArticle(articleStartRow, articlePageSize);
		if(pageArticle != null) {
			for(Article article : pageArticle) {
				...
				List<ArticleImg> currenArticleImg = articleControl.queryArticleImgByArticleId(article_id);
				ArticleImg firstImg  = currenArticleImg.get(0);
				map.put("image_id", firstImg.getImage_id());
				map.put("image_url", firstImg.getImage_url());
				//获取该文章评论数量
				Long commCount = articleControl.queryCommCount(article_id);
	...
	public List<Map<String, Object>> getOneArticleInfo(Integer article_id) throws SQLException {
		List<Map<String, Object>> list = new ArrayList<Map<String,Object>>();
		Article oneArticle = articleControl.queryArticleById(article_id);
		if(oneArticle != null) {
			Map<String, Object> map = new HashMap<String, Object>();
			...
			map.put("commCount", commCount);
			//获得该文章所有评论
			List<Comment> currenComment = articleControl.queryAllComment(article_id);
	...
	public List<Map<String, Object>> getPageCommInfo(Integer commPresentPage,Integer article_id) throws SQLException{
		...
		List<Comment> pageComm = articleControl.queryPageComment(article_id, commStartRow, commPageSize);
		if(pageComm != null) {
			for(Comment comment : pageComm) {
				...
				Long replierCount = articleControl.queryReplierCount(comment_id);
				map.put("replierCount", replierCount);
				//获得该评论赞数量
				Long praiseCount = articleControl.queryPraiseCount(comment_id);
